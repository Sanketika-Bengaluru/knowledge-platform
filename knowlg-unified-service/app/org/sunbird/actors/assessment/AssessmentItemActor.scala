...
class AssessmentItemActor @Inject()(implicit oec: OntologyEngineContext) extends BaseActor {
  ...
  def read(request: Request): Future[Response] = {
    val fieldsParam = Option.getOrElse("")
    val requestedFields: util.List[String] = fieldsParam.split(",")
      .asScala.filter(field => StringUtils.isNotBlank(field) && !StringUtils.equalsIgnoreCase(field, "null"))
      .toList.asJava
    ...
    DataNode.read(request).asScala.map(node => {
      ...
      val bodyFromExternal = if (externalData != null && externalData.contains("body")) externalData.get("body") else null
