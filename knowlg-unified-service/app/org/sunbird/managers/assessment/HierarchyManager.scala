...
object HierarchyManager {
    ...
    def addLeafNodesToHierarchy(request:Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
        ...
        rootNodeFuture.asScala.map(rootNode => {
            val unitId = request.getRequest.getOrElse("collectionId", "").asInstanceOf[String]
            ...
            if (StringUtils.isBlank(unitId)) attachLeafToRootNode(request, rootNode, "add") else {
                ...
                val childNodes: List[String] = rootNodeMap.get("childNodes") match {
                    ...
                    case y: util.List[_] => y.asInstanceOf[util.List[String]].asScala.toList
                }
                if(!childNodes.contains(unitId)) {
                    ...
                    hierarchyFuture.asScala.map(hierarchy => {
                        if(hierarchy.isEmpty){
                            ...
                            leafNodesFuture.asScala.map(leafNodes => {
                                val requestContextVersion = request.getContext.getOrElse("version", "1.0").asInstanceOf[String].toDouble
                                if (requestContextVersion != 1.0 && requestContextVersion >= 1.1) {
                                    ...
                                    val filteredChildNodes = leafNodes.asScala.filter(node => rootNodeQumlVer != node.getMetadata.getOrDefault("qumlVersion", 1.0.asInstanceOf[AnyRef]))
                                    if (!filteredChildNodes.isEmpty)
                                        throw new ClientException("ERR_OBJECT_VALIDATION", s"Children with identifier ${filteredChildNodes.asScala.map(node => node.getIdentifier.replace(".img", "")).asJava} can't be added because they don't have data in QuML ${rootNodeQumlVer} format.")
                                }
                                updateHierarchyData(unitId, hierarchy, leafNodes, rootNode, request, "add").asScala.map(node => ResponseHandler.OK.put("rootId", node.getIdentifier.replaceAll(imgSuffix, "")))
                            }).flatMap(f => f)
                        }
                    }).flatMap(f => f)
                }
            ...
            if (StringUtils.isBlank(unitId)) attachLeafToRootNode(request, rootNode, "remove") else {
                ...
                if(!childNodes.contains(unitId)) {
                    ...
                    hierarchyFuture.asScala.map(hierarchy => {
                        if(hierarchy.isEmpty){
                            Future{ResponseHandler.ERROR(ResponseCode.SERVER_ERROR, ResponseCode.SERVER_ERROR.name(), "hierarchy is empty")}
                        } else updateHierarchyData(unitId, hierarchy, null, rootNode, request, "remove").asScala.map(node => ResponseHandler.OK.put("rootId", node.getIdentifier.replaceAll(imgSuffix, "")))
                    }).flatMap(f => f)
                }
            }
        }).flatMap(f => f) recoverWith {case e: CompletionException => throw e.getCause}
    ...
    def attachLeafToRootNode(request: Request, rootNode: Node, operation: String)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
        fetchHierarchy(request, rootNode.getIdentifier).asScala.map(hierarchy => {
            if (hierarchy.isEmpty) {
                Future(ResponseHandler.ERROR(ResponseCode.SERVER_ERROR, ResponseCode.SERVER_ERROR.name(), "hierarchy is empty"))
            } else {
                fetchLeafNodes(request).asScala.map(leafNodes => {
                    val schemaVersion = rootNode.getMetadata.getOrDefault("schemaVersion", "1.0").asInstanceOf[String]
                    ...
                    if (isBranchingEnabled(rootNodeMap, request, operation)) {
                        val branchingLogic = rootNodeMap.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
                        TelemetryManager.info(s"Branching Found For ${rootNode.getIdentifier}. Branching Rules are : ${branchingLogic}")
                        leafNodes.asScala.foreach(leafNode => {
                    ...
                    } else if (StringUtils.equalsIgnoreCase("add", operation) && !request.getRequest.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap[String, AnyRef]()).asInstanceOf[util.Map[String, AnyRef]].isEmpty)
                    ...
                    updateRootNode(rootNode, request, operation).asScala.map(node => {
                        updateRootHierarchy(hierarchy, leafNodes, node, request, operation).asScala.map(response => {
    ...
    def updateRootHierarchy(hierarchy: java.util.Map[String, AnyRef], leafNodes: List[Node], rootNode: Node, request: Request, operation: String)(implicit oec: OntologyEngineContext, ec: ExecutionContext) = {
        ...
        if ("remove".equalsIgnoreCase(operation)) {
            val filteredChildren = hierarchy.get("children")
                ...
                .asScala.filter(child => !leafNodeIds.contains(child.get("identifier")))
            filteredChildren.asScala.sortBy(_.get("index").asInstanceOf[Integer])
                .zipWithIndex.asScala.foreach(zippedChild => zippedChild._1.put("index", (zippedChild._2.asInstanceOf[Integer] + 1).asInstanceOf[Object]))
    ...
    def getUnPublishedHierarchy(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
        ...
        rootNodeFuture.asScala.map(rootNode => {
            ...
            hierarchy.asScala.map(hierarchy => {
                ...
                getLatestLeafNodes(leafNodeIds).asScala.map(leafNodesMap => {
            ...
            searchResponse.asScala.map(rootHierarchy => {
                if(!rootHierarchy.isEmpty && StringUtils.isNotEmpty(rootHierarchy.asInstanceOf[util.HashMap[String, AnyRef]].get("identifier").asInstanceOf[String])){
                    ...
                    unPublishedBookmarkHierarchy.asScala.map(hierarchy => {
                        if (!hierarchy.isEmpty) {
                            val children = hierarchy.getOrElse("children", new util.ArrayList[java.util.Map[String, AnyRef]]).asInstanceOf[util.ArrayList[java.util.Map[String, AnyRef]]]
                            val leafNodeIds = new util.ArrayList[String]()
                            fetchAllLeafNodes(children, leafNodeIds)
    ...
    def getPublishedHierarchy(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Response] = {
        ...
        hierarchyFuture.asScala.map(result => {
            if (!result.isEmpty) {
                val bookmarkId = request.get("bookmarkId").asInstanceOf[String]
    ...
    def validateRequest(request: Request, operation: String)(implicit ec: ExecutionContext) = {
        ...
        val branchingLogic = request.getRequest.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
    ...
    def fetchLeafNodes(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[List[Node]] =  {
        ...
        DataNode.list(req).asScala.map(nodes => {
            if(nodes.size() != leafNodes.size()) {
                val filteredList = leafNodes.asScala.toList.asScala.filter(id => !nodes.contains(id))
                throw new ClientException(ErrorCodes.ERR_BAD_REQUEST.name(), "Children which are not available are: " + filteredList)
            } else {
                val invalidNodes = nodes.asScala.filterNot(node => ASSESSMENT_OBJECT_TYPES.contains(node.getObjectType))
                if (CollectionUtils.isNotEmpty(invalidNodes))
                    throw new ClientException(ErrorCodes.ERR_BAD_REQUEST.name(), s"Children must be of types $ASSESSMENT_OBJECT_TYPES for ids:  ${invalidNodes.asScala.map(_.getIdentifier)}")
        ...
        leafNodes.asScala.map(node => {
            val updatedNode: Node = if (node.getObjectType.equalsIgnoreCase("QuestionSet")
              && node.getMetadata.getOrDefault("visibility", "Parent").asInstanceOf[String].equalsIgnoreCase("Parent")) {
                val extData = if (null != node.getExternalData) node.getExternalData.asScala.filter(entry => !externalKeys.contains(entry._1)).asJava else Map().asJava
    ...
    def addChildrenToUnit(children: java.util.List[java.util.Map[String,AnyRef]], unitId:String, leafNodes: java.util.List[java.util.Map[String, AnyRef]], leafNodeIds: java.util.List[String], request: Request): Unit = {
        val childNodes = children.asScala.asScala.filter(child => ("Parent".equalsIgnoreCase(child.get("visibility").asInstanceOf[String]) && unitId.equalsIgnoreCase(child.get("identifier").asInstanceOf[String]))).toList
        if(null != childNodes && !childNodes.isEmpty){
            ...
            if (isBranchingEnabled(child, request, "add")) {
                ...
                val childrenIds: List[String] = child.getOrElse(HierarchyConstants.CHILDREN, new util.ArrayList[java.util.Map[String, AnyRef]]()).asInstanceOf[util.ArrayList[java.util.Map[String, AnyRef]]].asScala.asScala.toList.asScala.map(child => child.get("identifier").asInstanceOf[String])
                leafNodeIds.asScala.foreach(nodeId => {
                    val updatedBranching = addBranching(nodeId, child.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap()).asInstanceOf[java.util.Map[String, AnyRef]], request, childrenIds)
                    if (MapUtils.isNotEmpty(updatedBranching)) {
            ...
            } else if(!request.getRequest.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap[String, AnyRef]()).asInstanceOf[util.Map[String, AnyRef]].isEmpty)
    ...
    def removeChildrenFromUnit(children: java.util.List[java.util.Map[String, AnyRef]], unitId: String, leafNodeIds: java.util.List[String]):Unit = {
        ...
        if(null != childNodes && !childNodes.isEmpty){
            ...
            if (isBranchingEnabled(child, new Request(), "remove")) {
                TelemetryManager.info(s"Branching Found for ${child.get("identifier")}. Branching Rules Are : ${child.get(HierarchyConstants.BRANCHING_LOGIC)}")
                leafNodeIds.asScala.foreach(nodeId => {
                    val updatedBranching = removeBranching(nodeId, child.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap()).asInstanceOf[java.util.Map[String, AnyRef]])
                    if (MapUtils.isNotEmpty(updatedBranching)) {
                        child.put(HierarchyConstants.BRANCHING_LOGIC, updatedBranching)
                    }
                })
            ...
            if(null != child.get("children") && !child.get("children").asInstanceOf[java.util.List[java.util.Map[String,AnyRef]]].isEmpty) {
                var filteredLeafNodes = child.get("children").asInstanceOf[java.util.List[java.util.Map[String,AnyRef]]].asScala.asScala.filter(existingLeafNode => {
                    !leafNodeIds.contains(existingLeafNode.get("identifier").asInstanceOf[String])
                }).toList
                var index: Integer = 1
                filteredLeafNodes.asScala.sortBy(x => x.get("index").asInstanceOf[Integer]).asScala.foreach(node => {
    ...
    def updateRootNode(rootNode: Node, request: Request, operation: String)(implicit oec: OntologyEngineContext, ec: ExecutionContext) = {
        ...
        if(request.getRequest.contains(HierarchyConstants.BRANCHING_LOGIC))
    ...
    def restructureUnit(childList: java.util.List[java.util.Map[String, AnyRef]], leafNodes: java.util.List[java.util.Map[String, AnyRef]], leafNodeIds: java.util.List[String], depth: Integer, parent: String): java.util.List[java.util.Map[String, AnyRef]] = {
        ...
        if(null != childList && !childList.isEmpty) {
            val childMap:Map[String, java.util.Map[String, AnyRef]] = childList.asScala.asScala.toList.asScala.map(f => f.get("identifier").asInstanceOf[String] -> f).toMap
            val existingLeafNodes = childMap.asScala.filter(p => leafNodeIds.contains(p._1))
            existingLeafNodes.asScala.map(en => {
                leafNodeMap.get(en._1).put("index", en._2.get("index").asInstanceOf[Integer])
            })
            filteredLeafNodes = childList.asScala.asScala.filter(existingLeafNode => {
                !leafNodeIds.contains(existingLeafNode.get("identifier").asInstanceOf[String])
            }).toList.asJava
            maxIndex = childMap.values.asScala.map(child => child.get("index").asInstanceOf[Integer]).max.asInstanceOf[Integer]
        }
        leafNodeIds.asScala.foreach(id => {
            var node = leafNodeMap.getOrElse(id, new util.HashMap[String, AnyRef]())
    ...
    def fetchHierarchy(request: Request, identifier: String)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Map[String, AnyRef]] = {
        ...
        responseFuture.asScala.map(response => {
            if (!ResponseHandler.checkError(response)) {
    ...
    def getCassandraHierarchy(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[util.Map[String, AnyRef]] = {
        ...
        hierarchy.asScala.map(hierarchy => {
            if (!hierarchy.isEmpty) {
                ...
                val searchResponse = searchRootIdInElasticSearch(request.get("rootId").asInstanceOf[String])
                searchResponse.asScala.map(response => {
                    if (!response.isEmpty) {
                        if (StringUtils.isNotEmpty(response.getOrElse("identifier", "").asInstanceOf[String])) {
                            val parentHierarchy = fetchHierarchy(request, response.get("identifier").asInstanceOf[String])
                            parentHierarchy.asScala.map(hierarchy => {
                                if (!hierarchy.isEmpty) {
    ...
    def filterBookmarkHierarchy(children: util.List[util.Map[String, AnyRef]], bookmarkId: String)(implicit ec: ExecutionContext): util.Map[String, AnyRef] = {
        if (CollectionUtils.isNotEmpty(children)) {
            val response = children.asScala.asScala.filter(_.get("identifier") == bookmarkId).toList
    ...
    def updateLatestLeafNodes(children: util.List[util.Map[String, AnyRef]], leafNodeMap: util.Map[String, AnyRef]): List[Any] = {
        children.asScala.asScala.toList.asScala.map(content => {
            if(StringUtils.equalsIgnoreCase("Default", content.getOrDefault("visibility", "").asInstanceOf[String])) {
                val metadata: util.Map[String, AnyRef] = leafNodeMap.getOrElse(content.get("identifier").asInstanceOf[String], new java.util.HashMap[String, AnyRef]()).asInstanceOf[util.Map[String, AnyRef]]
    ...
    def getLatestLeafNodes(leafNodeIds : util.List[String])(implicit oec: OntologyEngineContext, ec: ExecutionContext) = {
        if(CollectionUtils.isNotEmpty(leafNodeIds)) {
            ...
            DataNode.list(request).asScala.map(nodes => {
                val leafNodeMap: Map[String, AnyRef] = nodes.asScala.asScala.toList.asScala.map(node => (node.getIdentifier, NodeUtil.serialize(node, null, node.getObjectType.toLowerCase.replace("image", ""), HierarchyConstants.SCHEMA_VERSION, true).asInstanceOf[AnyRef])).toMap
                val imageNodeIds: util.List[String] = leafNodeIds.asScala.asScala.toList.asScala.map(id => id + HierarchyConstants.IMAGE_SUFFIX).asJava
                request.put("identifiers", imageNodeIds)
                DataNode.list(request).asScala.map(imageNodes => {
                    val imageLeafNodeMap: Map[String, AnyRef] = imageNodes.asScala.asScala.toList.asScala.map(imageNode => {
    ...
    def isBranchingEnabled(metadata: java.util.Map[String, AnyRef], request: Request, operation: String): Boolean = {
        val isBranchingAvailable = operation match {
            case "add" => MapUtils.isNotEmpty(request.getRequest.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap()).asInstanceOf[java.util.Map[String, AnyRef]])
    ...
    def removeBranching(identifier: String, branchingLogic: java.util.Map[String, AnyRef]): java.util.Map[String, AnyRef] = {
        if (branchingLogic.keySet().contains(identifier)) {
            val obj: java.util.Map[String, AnyRef] = branchingLogic.getOrElse(identifier, new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
            val source: java.util.List[String] = obj.getOrElse(HierarchyConstants.SOURCE, new java.util.ArrayList[String]()).asInstanceOf[java.util.List[String]]
            val target: java.util.List[String] = obj.getOrElse(HierarchyConstants.TARGET, new java.util.ArrayList[String]()).asInstanceOf[java.util.List[String]]
            val preCondition: java.util.Map[String, AnyRef] = obj.getOrElse(HierarchyConstants.PRE_CONDITION, new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
            if ((!source.isEmpty && !preCondition.isEmpty) && target.isEmpty) {
                val parentObj: java.util.Map[String, AnyRef] = branchingLogic.getOrElse(source.get(0), new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
                val pTarget = parentObj.getOrElse(HierarchyConstants.TARGET, new java.util.ArrayList[String]()).asInstanceOf[java.util.List[String]]
    ...
    def addBranching(identifier: String, branchingLogic: java.util.Map[String, AnyRef], request: Request, childrenIds: List[String]): java.util.Map[String, AnyRef] = {
        val reqBranching: util.Map[String, AnyRef] = request.getRequest.getOrElse(HierarchyConstants.BRANCHING_LOGIC, new util.HashMap[String, AnyRef]()).asInstanceOf[util.Map[String, AnyRef]]
        ...
        reqBranching.asScala.asScala.map(entry => {
            ...
            if (branchingLogic.contains(source.get(0))) {
                val parentObj = branchingLogic.getOrElse(source.get(0), new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
                val pSource: java.util.List[String] = parentObj.getOrElse(HierarchyConstants.SOURCE, new java.util.ArrayList[String]()).asInstanceOf[java.util.List[String]]
                if (!pSource.isEmpty)
                ...
                val pTarget: java.util.List[String] = parentObj.getOrElse(HierarchyConstants.TARGET, new java.util.ArrayList[String]()).asInstanceOf[java.util.List[String]]
