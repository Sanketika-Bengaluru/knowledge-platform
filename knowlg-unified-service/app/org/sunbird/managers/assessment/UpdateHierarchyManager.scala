...
object UpdateHierarchyManager {
    ...
    def updateHierarchy(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
        ...
        getValidatedRootNode(rootId, request).asScala.map(node => {
            getExistingHierarchy(request, node).asScala.map(existingHierarchy => {
                ...
                val nodes = List(node)
                addChildNodesInNodeList(existingChildren, request, nodes).asScala.map(list => (existingHierarchy, list))
            }).flatMap(f => f)
              .asScala.map(result => {
                  val nodes = result._2
                  ...
                  idMap += (rootId -> rootId)
                  updateNodesModifiedInNodeList(nodes, nodesModified, request, idMap).asScala.map(updatedList => {
                      val modifiedNodeList = processBranching(updatedList, request, idMap)
                      getChildrenHierarchy(request, modifiedNodeList, rootId, hierarchy, idMap, result._1).asScala.map(children => {
                          TelemetryManager.log("Children for root id :" + rootId +" :: " + JsonUtils.serialize(children))
                          updateHierarchyData(rootId, children, modifiedNodeList, request).asScala.map(node => {
                              val response = ResponseHandler.OK()
                              response.put(HierarchyConstants.IDENTIFIER, rootId)
                              idMap.remove(rootId)
                              response.put(HierarchyConstants.IDENTIFIERS, idMap.asJava)
                              if (request.getContext.getOrElse("shouldImageDelete", false.asInstanceOf[AnyRef]).asInstanceOf[Boolean])
    ...
    private def validateRequest(request: Request)(implicit ec: ExecutionContext): (java.util.HashMap[String, AnyRef], java.util.HashMap[String, AnyRef]) = {
        ...
        hierarchy.asScala.keys.asScala.foreach(key => {
            if (StringUtils.equalsIgnoreCase(nodesModified.getOrElse(key, new util.HashMap()).asInstanceOf[util.Map[String, AnyRef]]
                .getOrElse(HierarchyConstants.OBJECT_TYPE, "").asInstanceOf[String], "Question"))
    ...
    private def getValidatedRootNode(identifier: String, request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Node] = {
        ...
        DataNode.read(req).asScala.map(rootNode => {
    ...
    private def getExistingHierarchy(request: Request, rootNode: Node)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[java.util.HashMap[String, AnyRef]] = {
        fetchHierarchy(request, rootNode).asScala.map(hierarchyString => {
    ...
    private def fetchHierarchy(request: Request, rootNode: Node)(implicit ec: ExecutionContext, oec:OntologyEngineContext): Future[Any] = {
        ...
        oec.graphService.readExternalProps(req, List(HierarchyConstants.HIERARCHY)).asScala.map(response => {
            if (ResponseHandler.checkError(response) && ResponseHandler.isResponseNotFoundError(response)) {
                if (CollectionUtils.containsAny(HierarchyConstants.HIERARCHY_LIVE_STATUS, rootNode.getMetadata.get("status").asInstanceOf[String]))
                    ...
                    if (rootNode.getMetadata.contains("pkgVersion"))
                    ...
                    oec.graphService.readExternalProps(req, List(HierarchyConstants.HIERARCHY)).asScala.map(resp => {
            ...
            } else Future(response.getResult.asScala.toMap.getOrElse(HierarchyConstants.HIERARCHY, "").asInstanceOf[String])
        }).flatMap(f => f)
    }
    ...
    private def addChildNodesInNodeList(childrenMaps: java.util.List[java.util.Map[String, AnyRef]], request: Request, nodes: scala.collection.immutable.List[Node])(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[scala.collection.immutable.List[Node]] = {
        if (CollectionUtils.isNotEmpty(childrenMaps)) {
            val futures = childrenMaps.asScala.map(child => {
                addNodeToList(child, request, nodes).asScala.map(modifiedList => {
            ...
            Future.sequence(futures).asScala.map(f => f.flatten.distinct)
        } else {
            Future(nodes)
        }
    }
    ...
    private def addNodeToList(child: java.util.Map[String, AnyRef], request: Request, nodes: scala.collection.immutable.List[Node])(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[scala.collection.immutable.List[Node]] = {
        if (StringUtils.isNotEmpty(child.get(HierarchyConstants.VISIBILITY).asInstanceOf[String]))
            if (StringUtils.equalsIgnoreCase(HierarchyConstants.DEFAULT, child.get(HierarchyConstants.VISIBILITY).asInstanceOf[String])) {
                getQuestionNode(child.getOrElse(HierarchyConstants.IDENTIFIER, "").asInstanceOf[String], HierarchyConstants.TAXONOMY_ID, child.getOrElse("schemaVersion", "1.0").asInstanceOf[String]).asScala.map(node => {
    ...
    private def updateNodesModifiedInNodeList(nodeList: List[Node], nodesModified: java.util.HashMap[String, AnyRef], request: Request, idMap: mutable.Map[String, String])(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[List[Node]] = {
        ...
        val futures = nodesModified.asScala.filter(nodeModified => !StringUtils.startsWith(request.getContext.get(HierarchyConstants.ROOT_ID).asInstanceOf[String], nodeModified._1))
                .asScala.map(nodeModified => {
                    val objectType = nodeModified._2.asInstanceOf[java.util.HashMap[String, AnyRef]].getOrElse(HierarchyConstants.OBJECT_TYPE, "").asInstanceOf[String]
                    ...
                    val metadata = nodeModified._2.asInstanceOf[java.util.HashMap[String, AnyRef]].getOrElse(HierarchyConstants.METADATA, new java.util.HashMap()).asInstanceOf[java.util.HashMap[String, AnyRef]]
                    ...
                    if (nodeModified._2.asInstanceOf[java.util.HashMap[String, AnyRef]].contains(HierarchyConstants.IS_NEW)
                            && nodeModified._2.asInstanceOf[java.util.HashMap[String, AnyRef]].get(HierarchyConstants.IS_NEW).asInstanceOf[Boolean]) {
                        if (!nodeModified._2.asInstanceOf[java.util.HashMap[String, AnyRef]].get(HierarchyConstants.ROOT).asInstanceOf[Boolean] && nodeModified._2.asInstanceOf[java.util.Map[String, AnyRef]].getOrElse(HierarchyConstants.METADATA, new util.HashMap()).asInstanceOf[java.util.Map[String, AnyRef]].getOrElse(HierarchyConstants.VISIBILITY, "").asInstanceOf[String].isEmpty)
        ...
        if (CollectionUtils.isNotEmpty(futures))
            Future.sequence(futures.toList).asScala.map(f => f.flatten)
    ...
    private def updateRootNode(rootId: String, nodeList: List[Node], nodesModified: java.util.HashMap[String, AnyRef])(implicit ec: ExecutionContext): Unit = {
        if (nodesModified.contains(rootId)) {
            val metadata = nodesModified.getOrElse(rootId, new java.util.HashMap()).asInstanceOf[java.util.HashMap[String, AnyRef]].getOrElse(HierarchyConstants.METADATA, new java.util.HashMap()).asInstanceOf[java.util.HashMap[String, AnyRef]]
    ...
    private def createNewNode(nodeId: String, idMap: mutable.Map[String, String], metadata: java.util.HashMap[String, AnyRef], nodeList: List[Node], request: Request, setDefaultValue: Boolean = true)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[List[Node]] = {
        ...
        if (neo4jCreateTypes.contains(objectType)) {
            ...
            DataNode.create(createRequest).asScala.map(node => {
            ...
            DefinitionNode.validate(createRequest, setDefaultValue).asScala.map(node => {
                node.setGraphId(HierarchyConstants.TAXONOMY_ID)
                node.setNodeType(HierarchyConstants.DATA_NODE)
                node.setObjectType(objectType)
                val updatedList = node :: nodeList
    ...
    private def updateTempNode(request:Request, nodeId: String, nodeList: List[Node], idMap: mutable.Map[String, String], metadata: java.util.HashMap[String, AnyRef])(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[List[Node]] = {
        ...
        if(null == tempNode)
            ...
            if (neo4jCreateTypes.contains(objectType)) {
                ...
                DataNode.update(createRequest).asScala.map(node => {
    ...
    private def validateNodes(nodeList: java.util.List[Node], rootId: String, request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[List[Node]] = {
        val nodesToValidate = nodeList.asScala.filter(node => (StringUtils.equals(HierarchyConstants.PARENT, node.getMetadata.get(HierarchyConstants.VISIBILITY).asInstanceOf[String])
    ...
    def constructHierarchy(list: List[java.util.Map[String, AnyRef]]): java.util.Map[String, AnyRef] = {
        val hierarchy: java.util.Map[String, AnyRef] = list.asScala.filter(root => root.get(HierarchyConstants.DEPTH).asInstanceOf[Number].intValue() == 0).head
        if (MapUtils.isNotEmpty(hierarchy)) {
            val maxDepth = list.asScala.map(node => node.get(HierarchyConstants.DEPTH).asInstanceOf[Number].intValue()).max
            for (i <- 0 to maxDepth) {
                val depth = i
                val currentLevelNodes: Map[String, List[java.util.Map[String, Object]]] = list.asScala.filter(node => node.get(HierarchyConstants.DEPTH).asInstanceOf[Number].intValue() == depth).groupBy(_.get("identifier").asInstanceOf[String].replaceAll(".img", ""))
                val nextLevel: List[java.util.Map[String, AnyRef]] = list.asScala.filter(node => node.get(HierarchyConstants.DEPTH).asInstanceOf[Number].intValue() == (depth + 1))
                if (CollectionUtils.isNotEmpty(nextLevel) && MapUtils.isNotEmpty(currentLevelNodes)) {
                    nextLevel.asScala.foreach(e => {
                        val parentId = e.get("parent").asInstanceOf[String]
                        currentLevelNodes.getOrElse(parentId, List[java.util.Map[String, AnyRef]]()).asScala.foreach(parent => {
                            val children = parent.getOrElse(HierarchyConstants.CHILDREN, new java.util.ArrayList[java.util.Map[String, AnyRef]]()).asInstanceOf[java.util.List[java.util.Map[String, AnyRef]]]
                            children.add(e)
    ...
    private def getChildrenHierarchy(request: Request, nodeList: List[Node], rootId: String, hierarchyData: java.util.HashMap[String, AnyRef], idMap: mutable.Map[String, String], existingHierarchy: java.util.Map[String, AnyRef])(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[java.util.List[java.util.Map[String, AnyRef]]] = {
        ...
        getPreparedHierarchyData(request, nodeList, rootId, childrenIdentifiersMap).asScala.map(nodeMaps => {
            TelemetryManager.info("prepared hierarchy list without filtering: " + nodeMaps.size())
            val filteredNodeMaps = nodeMaps.asScala.filter(nodeMap => null != nodeMap.get(HierarchyConstants.DEPTH)).toList
            ...
            if (MapUtils.isNotEmpty(hierarchyMap)) {
                hierarchyMap.getOrElse(HierarchyConstants.CHILDREN, new java.util.ArrayList[java.util.Map[String, AnyRef]]()).asInstanceOf[java.util.List[java.util.Map[String, AnyRef]]]
                        .asScala.filter(child => MapUtils.isNotEmpty(child))
            }
            else
    ...
    private def getChildrenIdentifiersMap(hierarchyData: java.util.Map[String, AnyRef], idMap: mutable.Map[String, String], existingHierarchy: java.util.Map[String, AnyRef]): Map[String, Map[String, Int]] = {
        if (MapUtils.isNotEmpty(hierarchyData)) {
            hierarchyData.asScala.map(entry => idMap.getOrElse(entry._1, entry._1) -> entry._2.asInstanceOf[java.util.HashMap[String, AnyRef]]
                    .get(HierarchyConstants.CHILDREN).asInstanceOf[java.util.ArrayList[String]]
                    .asScala.map(id => idMap.getOrElse(id, id)).zipWithIndex.toMap).toMap
        } else {
    ...
    private def getChildrenIdMapFromExistingHierarchy(existingHierarchy: java.util.Map[String, AnyRef], tempChildMap: java.util.Map[String, Map[String, Int]], tempResourceMap: java.util.Map[String, Map[String, Int]]): Unit = {
        if (existingHierarchy.contains(HierarchyConstants.CHILDREN) && CollectionUtils.isNotEmpty(existingHierarchy.get(HierarchyConstants.CHILDREN).asInstanceOf[java.util.ArrayList[java.util.HashMap[String, AnyRef]]])) {
            tempChildMap.put(existingHierarchy.get(HierarchyConstants.IDENTIFIER).asInstanceOf[String], existingHierarchy.get(HierarchyConstants.CHILDREN).asInstanceOf[java.util.ArrayList[java.util.HashMap[String, AnyRef]]]
                    .asScala.map(child => child.get(HierarchyConstants.IDENTIFIER).asInstanceOf[String] -> child.get(HierarchyConstants.INDEX).asInstanceOf[Int]).toMap)
            existingHierarchy.get(HierarchyConstants.CHILDREN).asInstanceOf[java.util.ArrayList[java.util.HashMap[String, AnyRef]]]
                    .asScala.foreach(child => getChildrenIdMapFromExistingHierarchy(child, tempChildMap, tempResourceMap))
        } else
    ...
    private def getPreparedHierarchyData(request: Request, nodeList: List[Node], rootId: String, childrenIdentifiersMap: Map[String, Map[String, Int]])(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[java.util.List[java.util.Map[String, AnyRef]]] = {
        if (MapUtils.isNotEmpty(childrenIdentifiersMap)) {
            ...
            updateHierarchyRelatedData(request, childrenIdentifiersMap.getOrElse(rootId, Map[String, Int]()), 1,
                rootId, nodeList, childrenIdentifiersMap, updatedNodeList).asScala.map(finalEnrichedNodeList => {
                TelemetryManager.info("Final enriched list size: " + finalEnrichedNodeList.size)
                val childNodeIds = finalEnrichedNodeList.asScala.map(node => node.getIdentifier.replaceAll(".img", "")).asScala.filterNot(id => StringUtils.containsIgnoreCase(rootId, id)).distinct
                ...
                validateNodes(finalEnrichedNodeList, rootId, request).asScala.map(result => HierarchyManager.convertNodeToMap(finalEnrichedNodeList))
            }).flatMap(f => f)
        } else {
            updateNodeList(nodeList, rootId, new java.util.HashMap[String, AnyRef]() {
                {
            ...
            validateNodes(nodeList, rootId, request).asScala.map(result => HierarchyManager.convertNodeToMap(nodeList))
        }
    }
    ...
    private def updateHierarchyRelatedData(request: Request, childrenIds: Map[String, Int], depth: Int, parent: String, nodeList: List[Node], hierarchyStructure: Map[String, Map[String, Int]], enrichedNodeList: scala.collection.immutable.List[Node])(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[List[Node]] = {
        val futures = childrenIds.asScala.map(child => {
            val id = child._1
            ...
            if (null != tempNode && StringUtils.equalsIgnoreCase(HierarchyConstants.PARENT, tempNode.getMetadata.get(HierarchyConstants.VISIBILITY).asInstanceOf[String])) {
                ...
                if (MapUtils.isNotEmpty(hierarchyStructure.getOrElse(child._1, Map[String, Int]())))
                    updateHierarchyRelatedData(request, hierarchyStructure.getOrElse(child._1, Map[String, Int]()),
                ...
                getQuestionNode(id, HierarchyConstants.TAXONOMY_ID, request.getContext.getOrElse("version", "1.0").asInstanceOf[String]).asScala.map(node => {
                    ...
                    if (MapUtils.isNotEmpty(hierarchyStructure.getOrElse(id, Map[String, Int]()))) {
                        updateHierarchyRelatedData(request, hierarchyStructure.getOrElse(id, Map[String, Int]()), node.getMetadata.get(HierarchyConstants.DEPTH).asInstanceOf[Int] + 1, id, nodeList, hierarchyStructure, nxtEnrichedNodeList)
                    } else
                        Future(nxtEnrichedNodeList)
        ...
        if (CollectionUtils.isNotEmpty(futures)) {
            ...
            listOfFutures.asScala.map(f => f.flatten.distinct)
        } else
    ...
    def isFullyPopulated(children: List[String], populatedChildMap: mutable.Map[_, _]): Boolean = {
        children.forall(child => populatedChildMap.contains(child))
    }
    ...
    def updateHierarchyData(rootId: String, children: java.util.List[java.util.Map[String, AnyRef]], nodeList: List[Node], request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Node] = {
        ...
        val prevStatus = node.getMetadata().getOrElse("status", "").asInstanceOf[String]
    ...
    private def cleanUpRootData(node: Node)(implicit oec: OntologyEngineContext, ec: ExecutionContext): java.util.Map[String, AnyRef] = {
        DefinitionNode.getRestrictedProperties(HierarchyConstants.TAXONOMY_ID, HierarchyConstants.SCHEMA_VERSION, HierarchyConstants.OPERATION_UPDATE_HIERARCHY, HierarchyConstants.QUESTIONSET_SCHEMA_NAME)
          .asScala.foreach(key => node.getMetadata.remove(key))
    ...
    def sortByIndex(childrenMaps: java.util.List[java.util.Map[String, AnyRef]]): java.util.List[java.util.Map[String, AnyRef]] = {
        childrenMaps.asScala.sortBy(_.get("index").asInstanceOf[Int]).asJava
    }
    ...
    private def processBranching(nodeList: List[Node], request: Request, idMap: mutable.Map[String, String])(implicit ec: ExecutionContext, oec: OntologyEngineContext): List[Node] = {
        val hierarchy: java.util.HashMap[String, AnyRef] = request.getRequest.get(HierarchyConstants.HIERARCHY).asInstanceOf[java.util.HashMap[String, AnyRef]]
        nodeList.distinct.asScala.map(node => {
    ...
    private def validateAndUpdateBranching(node: Node, hierarchy: java.util.Map[String, AnyRef], idMap: mutable.Map[String, String]): Node = {
        ...
        val updatedBranchingLogic = branchingLogic.asScala.map(entry => {
            ...
            val children = hierarchy.getOrElse(node.getIdentifier, new util.HashMap()).asInstanceOf[java.util.HashMap[String, AnyRef]].getOrElse(HierarchyConstants.CHILDREN, new util.ArrayList[String]()).asInstanceOf[java.util.List[String]].toList
            ...
            obj.put(HierarchyConstants.TARGET, target.asScala.map(id => idMap.getOrElse(id, id)).asJava)
            obj.put(HierarchyConstants.SOURCE, source.asScala.map(id => idMap.getOrElse(id, id)).asJava)
