...
object AssessmentManager {
	...
	def privateRead(request: Request, resName: String)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
		...
		DataNode.read(request).asScala.map(node => {
			val metadata: util.Map[String, AnyRef] = NodeUtil.serialize(node, fields, node.getObjectType.toLowerCase.replace("Image", ""), request.getContext.get("version").asInstanceOf[String])
			metadata.put("identifier", node.getIdentifier.replace(".img", ""))
				if (StringUtils.equalsIgnoreCase(metadata.getOrDefault("channel", "").asInstanceOf[String],request.getRequest.getOrElse("channel", "").asInstanceOf[String])) {
					ResponseHandler.OK.put(resName, metadata)
				}
				else {
	...
	def validateQuestionNodeForReview(request: Request, node: Node)(implicit ec: ExecutionContext, oec: OntologyEngineContext): List[String] = {
		...
		val metadata:util.Map[String, AnyRef] = metadataMap.entrySet().asScala.asScala.filter(entry => null != entry.getValue).asScala.map((entry: util.Map.Entry[String, AnyRef]) => handleKeyNames(entry, extPropNameList) ->  convertJsonProperties(entry, jsonProps)).toMap.asJava
	...
	def getChildIdsFromRelation(node: Node): (List[String], List[String]) = {
		...
		val visibilityIdMap: Map[String, List[String]] = outRelations
		  ...
		  .view.mapValues(_.asScala.map(_.getEndNodeId).toList).toMap
