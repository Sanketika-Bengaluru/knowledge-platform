...
import org.sunbird.utils.assessment.{AssessmentConstants, BranchingUtil, HierarchyConstants}
...
object CopyManager {
  ...
  def copy(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Response] = {
    ...
    DataNode.read(request).asScala.map(node => {
      ...
      copiedNodeFuture.asScala.map(copiedNode => {
  ...
  def validateExistingNode(request: Request, node: Node) = {
    ...
    val reqVer = request.getContext.getOrElse("version", "1.0").asInstanceOf[String]
  ...
  def copyQuestionSet(originNode: Node, request: Request)(implicit ex: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    ...
    copyNode(originNode, request).asScala.map(node => {
      ...
      HierarchyManager.getHierarchy(req).asScala.map(response => {
        val originHierarchy = response.getResult.getOrElse(AssessmentConstants.QUESTIONSET, new util.HashMap[String, AnyRef]()).asInstanceOf[java.util.Map[String, AnyRef]]
        copyType match {
  ...
  def copyNode(node: Node, request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    ...
    copyCreateReq.asScala.map(req => {
      DataNode.create(req).asScala.map(copiedNode => {
        Future(copiedNode)
      }).flatMap(f => f)
    }).flatMap(f => f)
  }
  ...
  def updateHierarchy(request: Request, node: Node, originNode: Node, originHierarchy: util.Map[String, AnyRef], copyType: String)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    prepareHierarchyRequest(originHierarchy, originNode, node, copyType, request).asScala.map(req => {
      ...
      UpdateHierarchyManager.updateHierarchy(updateHierarchyReq).asScala.map(response => {
        ...
        if (BLExists) {
          ...
          UpdateHierarchyManager.updateHierarchy(branchingUpdateReq).asScala.map(response_ => {
  ...
  def prepareHierarchyRequest(originHierarchy: util.Map[String, AnyRef], originNode: Node, node: Node, copyType: String, request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[util.Map[String, AnyRef]] = {
    ...
    if (null != children && !children.isEmpty) {
      ...
      getExternalData(idMap.keySet().asScala.toList, request).asScala.map(exData => {
        idMap.toMap.asScala.foreach(entry => {
          nodesModified.get(entry._2).asInstanceOf[java.util.Map[String, AnyRef]].get("metadata").asInstanceOf[util.Map[String, AnyRef]].putAll(exData.getOrElse(entry._1, new util.HashMap[String, AnyRef]()).asInstanceOf[util.Map[String, AnyRef]])
        })
  ...
  def getExternalData(identifiers: List[String], request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[util.Map[String, AnyRef]] = {
    ...
    val externalProps = DefinitionNode.getExternalProps(req.getContext.getOrElse("graph_id", "domain").asInstanceOf[String], req.getContext.getOrElse("version", "1.0").asInstanceOf[String], req.getContext.getOrElse("schemaName", "question").asInstanceOf[String])
    val externalPropsResponse = oec.graphService.readExternalProps(req, externalProps)
    externalPropsResponse.asScala.map(response => {
      identifiers.asScala.map(id => {
  ...
  def populateHierarchyRequest(children: util.List[util.Map[String, AnyRef]], nodesModified: util.HashMap[String, AnyRef], hierarchy: util.HashMap[String, AnyRef], parentId: String, copyType: String, request: Request, idMap: java.util.Map[String, String])(implicit ec: ExecutionContext, oec: OntologyEngineContext): Unit = {
    if (null != children && !children.isEmpty) {
      children.asScala.asScala.toList.asScala.foreach(child => {
          ...
          nodesModified.put(identifier, new util.HashMap[String, AnyRef]() {
            {
              put(AssessmentConstants.METADATA, cleanUpCopiedData(new util.HashMap[String, AnyRef]() {
                {
                  putAll(child)
                  put(AssessmentConstants.COPY_OF, child.getOrElse(AssessmentConstants.IDENTIFIER,""))
                  put(AssessmentConstants.CHILDREN, new util.ArrayList())
                  internalHierarchyProps.asScala.map(key => remove(key))
                }
              }, copyType))
              put(AssessmentConstants.ROOT, false.asInstanceOf[AnyRef])
              put(AssessmentConstants.OBJECT_TYPE, child.getOrElse(AssessmentConstants.OBJECT_TYPE, ""))
              put("isNew", true.asInstanceOf[AnyRef])
              put("setDefaultValue", false.asInstanceOf[AnyRef])
            }
          })
          if (StringUtils.equalsIgnoreCase(AssessmentConstants.QUESTION_MIME_TYPE, child.getOrElse("mimeType", "").asInstanceOf[String]))
            idMap.put(child.getOrElse("identifier", "").asInstanceOf[String], identifier)
          identifier
        } else
          child.get(AssessmentConstants.IDENTIFIER).asInstanceOf[String]
        if (StringUtils.equalsIgnoreCase(child.getOrElse(AssessmentConstants.MIME_TYPE, "").asInstanceOf[String], AssessmentConstants.QUESTIONSET_MIME_TYPE))
  ...
  def updateShallowHierarchy(request: Request, node: Node, originNode: Node, originHierarchy: util.Map[String, AnyRef])(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    ...
    val req = new Request(request)
    req.getContext.put(AssessmentConstants.SCHEMA_NAME, request.getContext.getOrElse(AssessmentConstants.SCHEMA_NAME, AssessmentConstants.QUESTIONSET_SCHEMA_NAME))
    req.getContext.put(AssessmentConstants.VERSION, request.getContext.getOrElse(AssessmentConstants.VERSION, AssessmentConstants.SCHEMA_VERSION))
    ...
    DataNode.update(req).asScala.map(node => node)
  }

  def getCopyRequest(node: Node, request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Request] = {
    val metadata: util.Map[String, AnyRef] = NodeUtil.serialize(node, new util.ArrayList(), node.getObjectType.toLowerCase.replace("image", ""), request.getContext.getOrElse(AssessmentConstants.VERSION, "").asInstanceOf[String])
    ...
    DataNode.read(readReq).asScala.map(node => {
      ...
      externalProps.asScala.foreach(prop => {
        val propValue = metadata.get(prop)
        if (metadata.contains(prop) && propValue != null) {
  ...
  def getOriginData(metadata: util.Map[String, AnyRef], copyType: String): java.util.Map[String, AnyRef] = (Map(AssessmentConstants.COPY_TYPE -> copyType) ++ originMetadataKeys.asScala.asScala.filter(key => metadata.contains(key)).asScala.map(key => key -> metadata.get(key)).toMap).asJava

  def validateRequest(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Unit = {
    val keysNotPresent = AssessmentConstants.REQUIRED_KEYS.asScala.filter(key => emptyCheckFilter(request.getRequest.getOrElse(key, "")))
  ...
  def cleanUpCopiedData(metadata: util.Map[String, AnyRef], copyType: String): util.Map[String, AnyRef] = {
    if (StringUtils.equalsIgnoreCase(AssessmentConstants.COPY_TYPE_SHALLOW, copyType)) {
      metadata.keySet().removeAll(metadataNotTobeCopied.asScala.toList.asScala.filter(str => !str.contains("dial")).asJava)
