...
object AssessmentV5Manager {
  ...
  def validateVisibilityForCreate(request: Request, errCode: String): Unit = {
    val visibility: String = request.getRequest.getOrElse("visibility", "").asInstanceOf[String]
  ...
  def create(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
    ...
    val qumlVersion = request.getRequest.getOrElse("qumlVersion", 0.0.asInstanceOf[AnyRef])
    ...
    DataNode.create(request).asScala.map(node => {
  ...
  def convertOneOfProps(node: Node, metadata: util.Map[String, AnyRef])(implicit oec: OntologyEngineContext, ec: ExecutionContext): util.Map[String, AnyRef] = {
    ...
    oneOfProps.asScala.foreach(key => {
  ...
  def getValidateNodeForReject(request: Request, errCode: String)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    ...
    DataNode.read(request).asScala.map(node => {
  ...
  def getNodeWithExternalProps(request: Request)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[Node] = {
    ...
    DataNode.read(readReq).asScala.map(node => node)
  }
  ...
  def validateHierarchy(request: Request, children: util.List[util.Map[String, AnyRef]], rootUserId: String)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Unit = {
    children.asScala.toList.asScala.foreach(content => {
      ...
      if ((StringUtils.equalsAnyIgnoreCase("application/vnd.sunbird.question", content.get("mimeType").toString) &&
        ...
        readReq.put("fields", extPropNameList)
        val messages: List[String] = Await.result(DataNode.read(readReq).asScala.map(node => {
  ...
  def updateHierarchy(hierarchy: util.Map[String, AnyRef], status: String, rootUserId: String): (java.util.Map[String, AnyRef], java.util.List[String]) = {
    ...
    val children = hierarchy.getOrElse("children", new util.ArrayList[java.util.Map[String, AnyRef]]).asInstanceOf[util.List[java.util.Map[String, AnyRef]]]
  ...
  def getQuestionSetHierarchy(request: Request, rootNode: Node)(implicit ec: ExecutionContext, oec: OntologyEngineContext): Future[String] = {
    ...
    HierarchyManager.getUnPublishedHierarchy(request).asScala.map(resp => {
  ...
  def processBooleanProps(data: util.Map[String, AnyRef]): Unit = {
    ...
    booleanProps.asScala.foreach(prop => {
  ...
  def processTimeLimits(data: util.Map[String, AnyRef]): Unit = {
    if (data.contains("timeLimits")) {
      val timeLimits:util.Map[String, AnyRef] = if(data.get("timeLimits").isInstanceOf[util.Map[_, _]]) data.getOrDefault("timeLimits", Map().asJava).asInstanceOf[util.Map[String, AnyRef]] else JsonUtils.deserialize(data.get("timeLimits").asInstanceOf[String], classOf[java.util.Map[String, AnyRef]])
  ...
  def getAnswer(data: util.Map[String, AnyRef]): String = {
    ...
    if (!StringUtils.equalsIgnoreCase(data.getOrDefault("primaryCategory", "").asInstanceOf[String], "Subjective Question") && !interactions.isEmpty) {
      ...
      val answerData = responseData.get("cardinality").asInstanceOf[String] match {
        case "single" => {
          val correctResp = responseData.getOrElse("correctResponse", Map().asJava).asInstanceOf[util.Map[String, AnyRef]].get("value").asInstanceOf[Integer]
          val label = options.toList.asScala.filter(op => op.get("value").asInstanceOf[Integer] == correctResp).head.get("label").asInstanceOf[String]
          val answer = """<div class="anwser-container"><div class="anwser-body">answer_html</div></div>""".replace("answer_html", label)
          answer
        }
        case "multiple" => {
          val correctResp = responseData.getOrElse("correctResponse", Map().asJava).asInstanceOf[util.Map[String, AnyRef]].get("value").asInstanceOf[util.List[Integer]]
          val singleAns = """<div class="anwser-body">answer_html</div>"""
          val answerList: List[String] = options.toList.asScala.filter(op => correctResp.contains(op.get("value").asInstanceOf[Integer])).asScala.map(op => singleAns.replace("answer_html", op.get("label").asInstanceOf[String])).toList
  ...
  def processHints(data: util.Map[String, AnyRef]): Unit = {
    ...
    if (!hints.isEmpty) {
      val updatedHints: util.Map[String, AnyRef] = hints.asScala.toList.asScala.map(hint => Map[String, AnyRef](UUID.randomUUID().toString -> hint).asJava).flatten.toMap.asJava
  ...
  def processResponseDeclaration(data: util.Map[String, AnyRef]): Unit = {
    ...
    if (StringUtils.equalsIgnoreCase("Subjective Question", data.getOrDefault("primaryCategory", "").toString)) {
      ...
      if(data.contains("maxScore") && null != data.get("maxScore")) {
      ...
      if (!responseDeclaration.isEmpty) {
        for (key <- responseDeclaration.keySet()) {
          ...
          val maxScore = Map[String, AnyRef]("cardinality" -> responseData.getOrElse("cardinality", "").asInstanceOf[String], "type" -> responseData.getOrElse("type", "").asInstanceOf[String], "defaultValue" -> responseData.get("maxScore"))
          responseData.remove("maxScore")
          outcomeDeclaration.put("maxScore", maxScore.asJava)
          //remove outcome from correctResponse
          responseData.getOrElse("correctResponse", Map[String, AnyRef]().asJava).asInstanceOf[util.Map[String, AnyRef]].remove("outcomes")
          ...
          try {
            if (StringUtils.equalsIgnoreCase("integer", responseData.getOrElse("type", "").asInstanceOf[String])
              && StringUtils.equalsIgnoreCase("single", responseData.getOrElse("cardinality", "").asInstanceOf[String])) {
              val correctResp: util.Map[String, AnyRef] = responseData.getOrElse("correctResponse", Map[String, AnyRef]().asJava).asInstanceOf[util.Map[String, AnyRef]]
              val correctKey: String = correctResp.getOrElse("value", "0").asInstanceOf[String]
          ...
          if (!mappingData.isEmpty) {
            val updatedMapping = mappingData.asScala.asScala.toList.asScala.map(mapData => {
              Map[String, AnyRef]("value" -> mapData.get("response"), "score" -> mapData.getOrElse("outcomes", Map[String, AnyRef]().asJava).asInstanceOf[util.Map[String, AnyRef]].get("score")).asJava
            }).asJava
  ...
  def processSolutions(data: util.Map[String, AnyRef]): Unit = {
    ...
    if (!solutions.isEmpty) {
      val updatedSolutions: util.Map[String, AnyRef] = solutions.asScala.asScala.toList.asScala.map(solution => {
        Map[String, AnyRef](solution.getOrElse("id", "").asInstanceOf[String] -> getSolutionString(solution, data.getOrDefault("media", List[util.Map[String, AnyRef]](Map[String, AnyRef]().asJava).asJava).asInstanceOf[util.List[util.Map[String, AnyRef]]]))
    ...
    if (!data.isEmpty) {
      data.getOrDefault("type", "").asInstanceOf[String] match {
        ...
        case "video" => {
          ...
          val mediaData: util.Map[String, AnyRef] = media.asScala.toList.asScala.filter(map => StringUtils.equalsIgnoreCase(value, map.getOrElse("id", "").asInstanceOf[String])).flatten.toMap.asJava
          val src = mediaData.getOrElse("src", "").asInstanceOf[String]
          val thumbnail = mediaData.getOrElse("thumbnail", "").asInstanceOf[String]
  ...
  def getTransformedHierarchy(data: util.Map[String, AnyRef]): util.Map[String, AnyRef] = {
    ...
    val children = updatedMeta.getOrElse("children", new util.ArrayList[java.util.Map[String, AnyRef]]).asInstanceOf[util.List[java.util.Map[String, AnyRef]]]
    if (!children.isEmpty)
  ...
  def tranformChildren(children: util.List[util.Map[String, AnyRef]]): Unit = {
    if (!children.isEmpty) {
      children.asScala.foreach(ch => {
        if (ch.contains("version")) ch.remove("version")
        processBloomsLevel(ch)
        processBooleanProps(ch)
        if(StringUtils.equalsIgnoreCase("application/vnd.sunbird.question", ch.getOrElse("mimeType", "").asInstanceOf[String]))
          ch.put("compatibilityLevel", 5.asInstanceOf[AnyRef])
        if (StringUtils.equalsIgnoreCase("application/vnd.sunbird.questionset", ch.getOrElse("mimeType", "").asInstanceOf[String])) {
          processTimeLimits(ch)
          processInstructions(ch)
          ch.put("compatibilityLevel", 6.asInstanceOf[AnyRef])
          val nestedChildren = ch.getOrElse("children", new util.ArrayList[java.util.Map[String, AnyRef]]).asInstanceOf[util.List[java.util.Map[String, AnyRef]]]
  ...
  def readComment(request: Request, resName: String)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
    val fields: util.List[String] = CollectionConverters.asJavaCollection(request.get("fields").asInstanceOf[String].split(",").asScala.filter(field => StringUtils.isNotBlank(field) && !StringUtils.equalsIgnoreCase(field, "null"))).asJava
