...
class QuestionSetActor @Inject()(implicit oec: OntologyEngineContext) extends AbstractActor {
  ...
  def privateRead(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
    val fields: util.List[String] = CollectionConverters.asJavaCollection(request.get("fields").asInstanceOf[String].split(",").asScala.filter(field => StringUtils.isNotBlank(field) && !StringUtils.equalsIgnoreCase(field, "null"))).asJava
    request.getRequest.put("fields", fields)
    if (StringUtils.isBlank(request.getRequest.getOrElse("channel", "").asInstanceOf[String])) throw new ClientException("ERR_INVALID_CHANNEL", "Please Provide Channel!")
    DataNode.read(request).asScala.map(node => {
      if (!StringUtils.equalsIgnoreCase(node.getMetadata.getOrDefault("channel", "").asInstanceOf[String], request.getRequest.getOrElse("channel", "").asInstanceOf[String]))
  ...
  def getHierarchy(request: Request)(implicit oec: OntologyEngineContext, ec: ExecutionContext): Future[Response] = {
    HierarchyManager.getHierarchy(request).asScala.map(resp => {
      if (StringUtils.equalsIgnoreCase(resp.getResponseCode.toString, "OK")) {
        ...
        val schemaVersion = updatedHierarchyMap.getOrElse("schemaVersion", "1.0").asInstanceOf[String]
  ...
  def reject(request: Request): Future[Response] = {
    ...
    AssessmentV5Manager.getValidateNodeForReject(request, AssessmentErrorCodes.ERR_OBJECT_VALIDATION).flatMap(node => {
      ...
      AssessmentV5Manager.getQuestionSetHierarchy(request, node).flatMap(hierarchyString => {
        ...
        val updatedMetadata = if (request.getRequest.contains("rejectComment")) (metadata ++ Map("rejectComment" -> request.get("rejectComment").asInstanceOf[String])) else metadata
  ...
  def publish(request: Request): Future[Response] = {
    val lastPublishedBy: String = request.getRequest.getOrElse("lastPublishedBy", "").asInstanceOf[String]
    val requestId = request.getContext().getOrElse("requestId","").asInstanceOf[String]
    val featureName = request.getContext().getOrElse("featureName","").asInstanceOf[String]
    request.getRequest.put("identifier", request.getContext.get("identifier"))
    request.put("mode", "edit")
    AssessmentV5Manager.getValidatedNodeForPublish(request, AssessmentErrorCodes.ERR_OBJECT_VALIDATION).flatMap(node => {
      AssessmentV5Manager.getQuestionSetHierarchy(request, node).asScala.map(hierarchyString => {
        val hierarchy = if (StringUtils.isNotBlank(hierarchyString)) JsonUtils.deserialize(hierarchyString, classOf[java.util.Map[String, AnyRef]]) else new util.HashMap[String, AnyRef]()
        val children = hierarchy.getOrElse("children", new util.ArrayList[java.util.Map[String, AnyRef]]).asInstanceOf[util.List[java.util.Map[String, AnyRef]]]
        if (children.isEmpty)
  ...
  def retire(request: Request): Future[Response] = {
    ...
    AssessmentV5Manager.getValidatedNodeForRetire(request, AssessmentErrorCodes.ERR_OBJECT_VALIDATION).flatMap(node => {
      ...
      DataNode.bulkUpdate(updateRequest).asScala.map(_ => {
  ...
  def update(request: Request): Future[Response] = {
    ...
    request.getRequest.put("identifier", request.getContext.get("identifier"))
    val versionKey = request.getRequest.getOrElse("versionKey","").asInstanceOf[String];
    ...
    DataNode.read(request).asScala.map(node => {
      ...
      DataNode.update(request).asScala.map(node => {
        ResponseHandler.OK.putAll(Map("identifier" -> node.getIdentifier.replace(".img", ""), "versionKey" -> node.getMetadata.get("versionKey")).asJava)
      })
    }).flatMap(f => f)
  }
  ...
  def systemUpdate(request: Request): Future[Response] = {
    ...
    }).asScala.map(node => ResponseHandler.OK.put("identifier", identifier).put("status", "success"))
  }
  ...
  def updateNode(request: Request, node: Node, metadata: Map[String, AnyRef]): Future[Response] = {
    ...
    DataNode.update(updateRequest).asScala.map(_ => {
      ResponseHandler.OK.putAll(Map("identifier" -> node.getIdentifier.replace(".img", ""), "versionKey" -> node.getMetadata.get("versionKey")).asJava)
    })
  }
  ...
  def updateComment(request: Request): Future[Response] = {
    ...
    val commentValue = request.getRequest.getOrElse("reviewComment", "").asInstanceOf[String]
