...
object AssessmentItemUtils {
  ...
  def validateAssessmentItemUsage(node: Node): Unit = {
    ...
    if (inRelations != null && !inRelations.isEmpty) {
      val activeRelations = inRelations.asScala.asScala.filter(rel =>
  ...
  def getSkipValidation(requestData: util.Map[String, AnyRef]): Boolean = {
    requestData.getOrElse("skipValidation", false.asInstanceOf[AnyRef]).asInstanceOf[Boolean]
  }

  def extractMetadata(requestData: util.Map[String, AnyRef]): util.Map[String, AnyRef] = {
    if (requestData.contains("metadata")) {
  ...
  def populateDefaults(metadata: util.Map[String, AnyRef]): Unit = {
    if (!metadata.contains("objectType")) metadata.put("objectType", "AssessmentItem")
    if (!metadata.contains("mimeType")) metadata.put("mimeType", "application/vnd.sunbird.assessmentitem")
    if (!metadata.contains("framework") || StringUtils.isBlank(metadata.get("framework").asInstanceOf[String])) metadata.put("framework", getDefaultFramework())
    if (!metadata.contains("version")) metadata.put("version", java.lang.Integer.valueOf(1))
    if (metadata.contains("level")) metadata.remove("level")
  }
  ...
  def flattenMetadataToRequest(request: Request, metadata: util.Map[String, AnyRef]): Unit = {
    ...
    if (!request.getRequest.contains("objectType")) {
