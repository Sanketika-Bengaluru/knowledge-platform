...
import org.sunbird.utils.assessment.AssessmentConstants
import java.util
...
object BranchingUtil {
	...
	def generateBranchingRecord(nodesModified: util.HashMap[String, AnyRef]): util.HashMap[String, AnyRef] = {
		...
		idSet.asScala.map(id => {
			val nodeMetaData = nodesModified.getOrElse(id, new util.HashMap()).asInstanceOf[util.Map[String, AnyRef]].getOrElse(AssessmentConstants.METADATA, new util.HashMap()).asInstanceOf[util.Map[String, AnyRef]]
			val containsBL = nodeMetaData.contains(AssessmentConstants.BRANCHING_LOGIC)
	...
	def hierarchyRequestModifier(request: Request, branchingRecord: util.HashMap[String, AnyRef], identifiers: util.Map[String, String]) = {
		...
		branchingRecord.keySet().asScala.asScala.toList.asScala.map(id => {
		...
		hierarchy.keySet().asScala.asScala.toList.asScala.map(id => {
			...
			children.asScala.asScala.map(identifier => {
				if (identifiers.contains(identifier)) newChildrenList.add(identifiers.get(identifier)) else newChildrenList.add(identifier)
			})
	...
	def branchingLogicModifier(branchingLogic: util.HashMap[String, AnyRef], oldToNewIdMap: util.Map[String, String]): Unit = {
		branchingLogic.keySet().asScala.asScala.toList.asScala.map(identifier => {
			val nodeBL = branchingLogic.get(identifier).asInstanceOf[util.HashMap[String, AnyRef]]
			nodeBL.keySet().asScala.asScala.toList.asScala.map(key => {
			...
			if (oldToNewIdMap.contains(identifier)) {
	...
	def branchingLogicArrayHandler(nodeBL: util.HashMap[String, AnyRef], name: String, oldToNewIdMap: util.Map[String, String]) = {
		val branchingLogicArray = nodeBL.getOrElse(name, new util.ArrayList[String]).asInstanceOf[util.ArrayList[String]]
		val newBranchingLogicArray = new util.ArrayList[String]()
		branchingLogicArray.asScala.asScala.map(id => {
			if (oldToNewIdMap.contains(id)) {
	...
	def preConditionHandler(nodeBL: util.HashMap[String, AnyRef], oldToNewIdMap: util.Map[String, String]): Unit = {
		...
		preCondition.keySet().asScala.asScala.toList.asScala.map(key => {
			...
			condition.keySet().asScala.asScala.toList.asScala.map(logicOp => {
				...
				if (oldToNewIdMap.contains(stringArray(0))) {
