...
abstract class BaseController(protected val cc: ControllerComponents)(implicit exec: ExecutionContext) extends AbstractController(cc) {
  ...
  def getResult(apiId: String, actor: ActorRef, request: org.sunbird.common.dto.Request) : Future[Result] = {
    ...
    future.asScala.map(f => {
      ...
      if(publishApis.containsKey(apiId)) {
        ...
        val params = Map("requestId" -> request.getContext().getOrElse("requestId", "").asInstanceOf[String], "Response Code" -> result.getResponseCode.code(), "cdata" -> Map("type"->"Feature", "id"-> request.getContext().getOrElse("featureName", "").asInstanceOf[String]).asJava).asJava.asInstanceOf[java.util.Map[String, AnyRef]]
        TelemetryManager.info(s"EXIT:assessment: ${apiName} | Response Provided For Identifier ${request.getContext.getOrElse("identifier", "").asInstanceOf[String]}.", params)
      }
